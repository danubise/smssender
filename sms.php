...
if(isset($_POST['send'])){
    $needsend = true;
    $errorMessage="";
    $smsto=trim($_POST['number']);
    $smstext = trim($_POST['smstext']);
    $sms_parts=0;
    if(strlen($smsto)<>11){
            $needsend=false;
            $errorMessage="Ошибка в номере '".$smsto."'";
    }

    if($needsend){
        $SmsCalc = new SmsLengthCalculator();

        $sms_parts = $SmsCalc->getPartCount($smstext);

        if($sms_parts>2){
            $needsend=false;
            $errorMessage="Превышено максивальное количество частей СМС '".$sms_parts." части/ей' ";
        }
        if($needsend){
            $smstext_edited = preg_replace('/(\r\n|\r|\n)+/', " ", $smstext);
            $url = "http://192.168.1.187/default/en_US/send.html?u=admin&p=adminpass&l=".rand(9, 12)."&n=".$smsto."&m=".htmlentities(urlencode($smstext_edited));
            //echo $url;
            $requestToGoip=file_get_contents($url);
...
<legend>Отправка смс:</legend>
    <form action="sms.php" method="post">
    <table>
        <tr>
            <td><p>Введите номер абонента</p></td>
            <td><input type="text" name="number" value="<?=$smsto?>"></td>
            <td>Номер в формате 7XXXXXXXXXX<br>(11 цифр, без знака +)</td>
        </tr>
        <tr>
            <td>Введите текст смс</td>
            <td><textarea name="smstext" maxlength="306" style="width: 148px; height: 150px;"><?=$smstext?></textarea></td>
            <td></td>
        </tr>
        <tr>
            <td>
            <?php
                if($needsend){
                    echo "<p>Смс отправлено на номер ".$smsto."</p><p>Количество частей ".$sms_parts."</p>";
                }else{
                    echo "<p><font color=\"red\">".$errorMessage."</font></p>";
                }
             ?>
            </td>
            <td><input type="submit" value="Отправить" name="send" ></td>
        </tr>
    </table>
    </form>
</fieldset>
